...
/*
        naverMap.locationOverlay.apply { // 커스텀 위치 아이콘 생성 함수
            isVisible = true
            icon = OverlayImage.fromResource(R.drawable.icon_arrow_back)
        }
        naverMap.getLocationOverlay()
 */

        naverMap.locationOverlay.circleRadius = 25 // 위치 오버레이 설정 코드
        naverMap.locationOverlay.circleColor = Color.WHITE
        naverMap.locationOverlay.circleOutlineColor = Color.BLUE
        naverMap.locationOverlay.circleOutlineWidth = 10
        ...
        if (hasPermission()) {
            ...
            naverMap.locationTrackingMode = LocationTrackingMode.NoFollow // 현재 위치를 나타내지 않는 기본 지도 상태 (None)
        }
        ...
        binding.btnCheck.setOnClickListener{ location -> // 버튼 클릭 시, 마커와 정해진 거리 이내에 위치해 있는지 확인 후, 토스트 메시지 출력 함수
            if(check_TUK == true){
