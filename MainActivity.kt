...
class MainActivity : AppCompatActivity() {
    ...
    private val KEY_UNLOCK_HAERO = "unlock_haero"
    private val KEY_UNLOCK_TINO = "unlock_tino"
    private val KEY_UNLOCK_TORO = "unlock_toro"
    private val KEY_UNLOCK_BACKGROUND = "unlock_background_" // + index (0~4)
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val selectedMascot = intent.getStringExtra("selected_mascot")
        if (selectedMascot != null) {
            val mascotIndex = if (selectedMascot == "toro") 1 else 0
        ...
        closetButton.setOnClickListener {
            val unlockedMascots = listOf(
                prefs.getBoolean(KEY_UNLOCK_HAERO, false),
                prefs.getBoolean(KEY_UNLOCK_TORO, false),
                prefs.getBoolean(KEY_UNLOCK_TINO, false)
            )
            val unlockedBackgrounds = (0..4).map {
                prefs.getBoolean("unlock_background_$it", it == 0) // 기본 배경(0번)은 항상 해금
            }
            ClosetDialog.show(
                this,
                ::changeMascot,
                { idx -> changeBackground(idx, backgrounds) },
                backgrounds,
                unlockedMascots = listOf(
                    prefs.getBoolean("unlock_haero", false),
                    prefs.getBoolean("unlock_toro", false),
                    prefs.getBoolean("unlock_tino", false)
                ),
    ...
    private fun changeBackground(index: Int, allBackgrounds: List<ImageView>) {
        allBackgrounds.forEachIndexed { i, bg ->
            bg.visibility = if (i == index) View.VISIBLE else View.GONE
        }
